...
import {SpotifyCard, SpotifyCompactCard, SpotifyIdleCard} from './spotify-card.js'

/**
 * Sidebar variant: always renders a labeled "Now playing" block, falling
 * back to an idle placeholder when nothing's playing (or the API hasn't
 * responded yet) so the sidebar layout doesn't jump around.
 */
  ...
  return (
    <div className={className}>
      ...
      {status === 'loading' ? (
        ...
        <SpotifyCompactCard spotify={track} />
      ) : (
        <SpotifyIdleCard />
      )}
    </div>
