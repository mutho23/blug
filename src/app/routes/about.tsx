  ...
  return (
    <div className="flex" style={{minHeight: 'calc(100vh - 52px - 48px)'}}>
      ...
      <aside className="hidden lg:block w-[200px] shrink-0 border-r border-[#1e1e1e] px-4 py-6">
        <p className="font-mono text-[9px] tracking-[0.12em] uppercase text-[#555] mb-3">Elsewhere</p>
        {[
          ...
          <a
            ...
            <span className="font-mono text-[10px] text-[#888] group-hover:text-[#f0f0f0] transition-colors truncate">
              {label}
            </span>
          </a>
        ))}

        <p className="font-mono text-[9px] tracking-[0.12em] uppercase text-[#555] mb-2 mt-5">Writing</p>
        <a href="/posts/why-me" className="block py-1.5 border-b border-[#1e1e1e] group">
          <div className="font-mono text-[10px] text-[#b0b0b0] group-hover:text-[#4a9eff] transition-colors">why me?</div>
          <div className="font-mono text-[9px] text-[#555] mt-0.5">May 30, 2026</div>
        </a>
      </aside>
      ...
      <div className="flex-1 px-6 md:px-10 py-9 max-w-2xl">
        ...
        <section className="mb-8">
          <h1 className="font-display text-[26px] md:text-[30px] text-[#f0f0f0] tracking-[-0.02em]">
            About Me<span className="text-[#4a9eff]">.</span>
          </h1>
          <p className="font-mono text-[12px] text-[#888] leading-[1.8] mt-2.5 max-w-[480px]">
...
function Section({label, children}: {label: string; children: React.ReactNode}) {
  return (
    <section className="border-t border-[#1e1e1e] pt-3.5 pb-4 mb-1">
      <p className="font-mono text-[9px] tracking-[0.12em] uppercase text-[#555] mb-3">{label}</p>
    ...
    <li className="flex items-start justify-between py-2 border-b border-[#1a1a1a] last:border-0">
      <div>
        <a
          ...
          rel="noopener noreferrer"
          className={`font-mono text-[12px] text-[#e0e0e0] ${href ? 'hover:text-[#4a9eff] transition-colors' : ''}`}>
          {company}
        </a>
        <div className="font-mono text-[9px] text-[#555] mt-0.5">{role}</div>
      </div>
      <span className="font-mono text-[9px] text-[#555] shrink-0 mt-0.5">{period}</span>
    </li>
  )
}
...
function LinkItem({name, href}: {name: string; href: string}) {
  return (
    <li className="py-2 border-b border-[#1a1a1a] last:border-0">
      <a
        href={href}
        className="group inline-flex items-center gap-1.5 font-mono text-[12px] text-[#888] hover:text-[#f0f0f0] transition-colors"
        target="_blank"
        rel="noreferrer">
        {name}
        <span className="text-[10px] text-[#444] opacity-0 group-hover:opacity-100 transition-opacity">↗</span>
      </a>
    </li>
  )
}
