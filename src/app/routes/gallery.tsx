...
// ─── wsrv.nl proxy settings ───────────────────────────────────────────────────
const THUMB_WIDTH = 400
const THUMB_QUALITY = 55
const FULL_WIDTH = 800
const FULL_QUALITY = 60

function toWsrv(blobUrl: string, opts: {w: number; q: number}): string {
...
// ─── Server-side DID cache ────────────────────────────────────────────────────
let cachedDid: string | null = null
let cachedDidAt = 0
const DID_CACHE_TTL_MS = 60 * 60 * 1000

async function resolveDid(): Promise<string> {
  const now = Date.now()
  if (cachedDid && now - cachedDidAt < DID_CACHE_TTL_MS) return cachedDid
...
/**
 * Cocokkan photo records ke gallery records.
 *
 * Grain menyimpan relasi photo→gallery, tapi tidak ada dokumentasi publik
 * soal field yang dipakai. Fungsi ini mencoba semua kemungkinan secara berurutan:
 *
 * Strategi 1 — gallery ref eksplisit di dalam photo record
 *   photo.value.gallery adalah AT-URI gallery (paling reliable)
 *
 * Strategi 2 — rkey match
 *   photo.value.galleryRkey === rkey gallery
 *
 * Strategi 3 — createdAt match (kode asli)
 *   Hanya cocok kalau foto dan gallery di-upload dalam waktu bersamaan persis.
 *   Ini fallback terakhir.
 */
function matchPhotosToGallery(
  photos: any[],
  galleryUri: string,
  galleryCreatedAt: string,
): any[] {
  const galleryRkey = galleryUri.split('/').at(-1)

  // Strategi 1: cek field gallery ref eksplisit (beberapa variasi nama field)
  const byRef = photos.filter(photo => {
    const v = photo.value
    const ref =
      v.gallery?.uri ??         // {gallery: {uri: 'at://...'}}
      v.gallery ??              // gallery: 'at://...' (string langsung)
      v.galleryRef?.uri ??      // {galleryRef: {uri: 'at://...'}}
      v.galleryRef ??           // galleryRef: 'at://...'
      v.galleryUri ??           // galleryUri: 'at://...'
      null
    if (typeof ref === 'string') return ref === galleryUri
    return false
  })
  if (byRef.length > 0) return byRef

  // Strategi 2: rkey match
  const byRkey = photos.filter(photo => {
    const v = photo.value
    const rkey =
      v.galleryRkey ??
      v.gallery?.rkey ??
      null
    return rkey != null && rkey === galleryRkey
  })
  if (byRkey.length > 0) return byRkey

  // Strategi 3: createdAt match (fallback ke perilaku asli)
  // Ini yang dipakai kode sebelumnya — tidak reliable untuk foto yang
  // di-upload belakangan, tapi tetap disimpan sebagai last resort.
  return photos.filter(photo => photo.value.createdAt === galleryCreatedAt)
}
...
export const loader = async () => {
  ...
  const [galleriesData, photosData] = await Promise.all([
    galleriesRes.json(),
    photosRes.json(),
  ])
  ...
  const galleries = (galleriesData.records ?? []).map((gallery: any) => {
    const matchedPhotos = matchPhotosToGallery(
      photos,
      gallery.uri,
      gallery.value.createdAt,
    )

    const images: ImageItem[] = matchedPhotos.map((photo: any) => {
      const blobUrl = `https://bsky.social/xrpc/com.atproto.sync.getBlob?did=${did}&cid=${photo.value.photo.ref.$link}`
      return {
        url: toWsrv(blobUrl, {w: THUMB_WIDTH, q: THUMB_QUALITY}),
        fullUrl: toWsrv(blobUrl, {w: FULL_WIDTH, q: FULL_QUALITY}),
  ...
  return json(
    {galleries, did},
    {
      headers: {
        Link: [
          '<https://bsky.social>; rel=preconnect',
          '<https://wsrv.nl>; rel=preconnect',
          '<https://bsky.social>; rel=dns-prefetch',
          '<https://wsrv.nl>; rel=dns-prefetch',
        ].join(', '),
      },
    },
  )
  ...
  return (
    <div className="relative h-full w-full overflow-hidden bg-gray-100">
      ...
      <img
        ...
        // @ts-expect-error fetchpriority valid HTML, belum ada di semua definisi TS
...
function Lightbox({
  ...
  useEffect(() => {
    ...
    const preload = (idx: number) => { new Image().src = images[idx].fullUrl }
    ...
    <div
      ...
      <div
        ...
        style={{
          ...
          width: aspectRatio >= 1 ? '90vw' : `calc(90vh * ${aspectRatio})`,
        }}
...
const MasonryGrid = memo(function MasonryGrid({
  ...
  if (images.length === 0) {
    return (
      <div className="flex h-48 w-full items-center justify-center bg-gray-100">
        <span className="font-mono text-xs text-gray-400">no photo</span>
      </div>
    )
  }
  ...
  gridImages.forEach((img, i) => {
    ;(i % 2 === 0 ? col1 : col2).push({...img, origIndex: i})
  })
            ...
            {col.map((img: any, i: number) => (
              <div
                ...
                <ProgressiveImage
                  ...
                  eager={eager}
                  priority={priority && colIdx === 0 && i === 0}
                  className="absolute inset-0 h-full w-full object-cover"
                />
              </div>
            ))}
    ...
    const observer = new IntersectionObserver(
      entries => {
        if (entries[0].isIntersecting) {
          setVisibleCount(prev => Math.min(prev + 6, galleries.length))
        }
      },
      {rootMargin: '800px'},
    )
    ...
    <article className="container mx-auto px-6 pb-24 pt-12 md:pt-20">
      ...
      {visibleGalleries.length === 0 ? (
        <div className="max-w-prose">
          <p className="font-sans text-lg text-500">Belum ada gallery.</p>
        </div>
      ) : (
        <>
                ...
                <div
                  ...
                  <MasonryGrid
                    ...
                    onPhotoClick={photoIndex =>
                      setLightbox({images: gallery.images, index: photoIndex})
                    }
                  />
