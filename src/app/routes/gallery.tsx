...
// Thumbnail: smaller width, lower quality — for grid display
// Full: wider cap, higher quality — for lightbox
...
function toWsrv(
  blobUrl: string,
  opts: {w: number; q: number; output?: string},
): string {
  // wsrv.nl expects the URL without the https:// scheme
  ...
  const params = new URLSearchParams({
    ...
    we: '1', // strip EXIF to reduce file size
  })
  return `https://wsrv.nl/?${params.toString()}`
}

type ImageItem = {
  /** Thumbnail URL (wsrv.nl proxy, low quality) — used in grid */
  url: string
  /** Full-resolution URL (wsrv.nl proxy, high quality) — used in lightbox */
  fullUrl: string
  width: number
  height: number
}
...
function Lightbox({
  ...
  return (
    <div
      ...
      {/* Show thumbnail instantly while full-res loads */}
      <div className="relative flex items-center justify-center" style={{maxHeight: '90vh', maxWidth: '90vw'}}>
        ...
        <img
          ...
          onClick={e => e.stopPropagation()}
          onLoad={e => {
            // Once full-res is loaded, hide the blurry thumbnail behind it
...
export default function Gallery() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      {
        // Larger margin so images start loading well before they scroll into view
        rootMargin: '800px',
      },
    )
