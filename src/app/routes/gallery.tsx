...
export const loader = async () => {
  ...
  const galleries = (galleriesData.records ?? []).map((gallery: any) => {
    ...
    const images = matchedPhotos.map((photo: any) => {
      ...
      return {
        // thumbnail ringan untuk gallery
        url: `https://cdn.bsky.app/img/feed_thumbnail/plain/${encodedDid}/${cid}@jpeg`,

        // medium quality fullscreen
        fullUrl: `https://cdn.bsky.app/img/feed_fullsize/plain/${encodedDid}/${cid}@jpeg`,
  ...
  return (
    <div
      ...
      {images.length > 1 && (
        <>
          <button
            className="hidden md:block absolute left-[2px] md:left-[calc(50%-430px)] top-1/2 z-30 -translate-y-1/2 rounded-full bg-blue-500/90 px-4 py-2 text-4xl font-bold text-white shadow-2xl hover:scale-110 hover:bg-blue-400 transition-all duration-200"
            onClick={e => {
              e.stopPropagation()
              prev()
            }}>
          ...
          <button
            className="hidden md:block absolute right-[2px] md:right-[calc(50%-430px)] top-1/2 z-30 -translate-y-1/2 rounded-full bg-blue-500/90 px-4 py-2 text-4xl font-bold text-white shadow-2xl hover:scale-110 hover:bg-blue-400 transition-all duration-200"
            onClick={e => {
              e.stopPropagation()
              next()
            }}>
