import {useLoaderData} from '@remix-run/react'
import {json} from '@remix-run/node'
import {
  useState,
  useEffect,
  useCallback,
  useRef,
  memo,
} from 'react'

const HANDLE = 'mutho.my.id'
const INITIAL_GALLERY_LIMIT = 6

// ─── wsrv.nl proxy settings ───────────────────────────────────────────────────
const THUMB_WIDTH = 400
const THUMB_QUALITY = 55
const FULL_WIDTH = 1200
const FULL_QUALITY = 75

function toWsrv(blobUrl: string, opts: {w: number; q: number}): string {
  const stripped = blobUrl.replace(/^https?:\/\//, '')
  const params = new URLSearchParams({
    url: stripped,
    w: String(opts.w),
    q: String(opts.q),
    output: 'webp',
    we: '1',
  })
  return `https://wsrv.nl/?${params.toString()}`
}
// ──────────────────────────────────────────────────────────────────────────────

type ImageItem = {
  url: string      // thumbnail via wsrv.nl — dipakai di grid
  fullUrl: string  // gambar besar via wsrv.nl — dipakai di lightbox
  width: number
  height: number
}

// ─── Server-side DID cache ────────────────────────────────────────────────────
// DID untuk sebuah handle hampir tidak pernah berubah.
// Menyimpannya di module-level memory memotong satu round-trip jaringan
// untuk setiap request setelah yang pertama (server tetap hidup selama proses Node berjalan).
let cachedDid: string | null = null
let cachedDidAt = 0
const DID_CACHE_TTL_MS = 60 * 60 * 1000 // 1 jam

async function resolveDid(): Promise<string> {
  const now = Date.now()
  if (cachedDid && now - cachedDidAt < DID_CACHE_TTL_MS) {
    return cachedDid
  }
  const res = await fetch(
    `https://bsky.social/xrpc/com.atproto.identity.resolveHandle?handle=${HANDLE}`,
  )
  const {did} = await res.json()
  cachedDid = did
  cachedDidAt = now
  return did
}
// ──────────────────────────────────────────────────────────────────────────────

export const loader = async () => {
  // DID di-cache — tidak perlu round-trip ke Bluesky setiap request
  const did = await resolveDid()

  // Fetch galleries dan photos paralel (sama seperti sebelumnya)
  const [galleriesRes, photosRes] = await Promise.all([
    fetch(
      `https://bsky.social/xrpc/com.atproto.repo.listRecords?repo=${did}&collection=social.grain.gallery&limit=30`,
    ),
    fetch(
      `https://bsky.social/xrpc/com.atproto.repo.listRecords?repo=${did}&collection=social.grain.photo&limit=100`,
    ),
  ])

  const [galleriesData, photosData] = await Promise.all([
    galleriesRes.json(),
    photosRes.json(),
  ])

  const photos: any[] = photosData.records ?? []

  // Buat lookup map photo by createdAt — O(1) per lookup, bukan O(n) filter
  const photosByTime = new Map<string, any[]>()
  for (const photo of photos) {
    const t = photo.value.createdAt
    if (!photosByTime.has(t)) photosByTime.set(t, [])
    photosByTime.get(t)!.push(photo)
  }

  const galleries = (galleriesData.records ?? []).map((gallery: any) => {
    const matchedPhotos = photosByTime.get(gallery.value.createdAt) ?? []

    const images = matchedPhotos.map((photo: any) => {
      const blobUrl = `https://bsky.social/xrpc/com.atproto.sync.getBlob?did=${did}&cid=${photo.value.photo.ref.$link}`
      return {
        url: toWsrv(blobUrl, {w: THUMB_WIDTH, q: THUMB_QUALITY}),
        fullUrl: toWsrv(blobUrl, {w: FULL_WIDTH, q: FULL_QUALITY}),
        width: photo.value.aspectRatio?.width ?? 1,
        height: photo.value.aspectRatio?.height ?? 1,
      }
    })

    return {...gallery, images}
  })

  return json(
    {galleries, did},
    {
      headers: {
        Link: [
          '<https://bsky.social>; rel=preconnect',
          '<https://wsrv.nl>; rel=preconnect',
          // dns-prefetch sebagai fallback untuk browser yang tidak support preconnect
          '<https://bsky.social>; rel=dns-prefetch',
          '<https://wsrv.nl>; rel=dns-prefetch',
        ].join(', '),
      },
    },
  )
}

function ProgressiveImage({
  src,
  alt,
  className,
  eager = false,
  priority = false,
}: {
  src: string
  alt: string
  className?: string
  eager?: boolean
  priority?: boolean
}) {
  const [loaded, setLoaded] = useState(false)
  const imgRef = useRef<HTMLImageElement>(null)

  useEffect(() => {
    if (imgRef.current?.complete) setLoaded(true)
  }, [])

  return (
    <div className="relative h-full w-full overflow-hidden bg-gray-100">
      {!loaded && (
        <div className="absolute inset-0 animate-pulse bg-gray-100" />
      )}
      <img
        ref={imgRef}
        src={src}
        alt={alt}
        loading={eager ? 'eager' : 'lazy'}
        decoding={priority ? 'sync' : 'async'}
        // @ts-expect-error fetchpriority valid HTML, belum ada di semua definisi TS
        fetchpriority={priority ? 'high' : eager ? 'auto' : 'low'}
        onLoad={() => setLoaded(true)}
        className={`
          ${className}
          transition-opacity duration-300
          ${loaded ? 'opacity-100' : 'opacity-0'}
        `}
      />
    </div>
  )
}

function Lightbox({
  images,
  initialIndex,
  onClose,
}: {
  images: ImageItem[]
  initialIndex: number
  onClose: () => void
}) {
  const [index, setIndex] = useState(initialIndex)
  const touchStartX = useRef<number | null>(null)

  const prev = useCallback(
    () => setIndex(i => (i - 1 + images.length) % images.length),
    [images.length],
  )
  const next = useCallback(
    () => setIndex(i => (i + 1) % images.length),
    [images.length],
  )

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') onClose()
      if (e.key === 'ArrowLeft') prev()
      if (e.key === 'ArrowRight') next()
    }
    window.addEventListener('keydown', onKey)
    return () => window.removeEventListener('keydown', onKey)
  }, [onClose, prev, next])

  useEffect(() => {
    document.body.style.overflow = 'hidden'
    return () => { document.body.style.overflow = '' }
  }, [])

  // Preload prev & next full-res
  useEffect(() => {
    if (images.length <= 1) return
    const preload = (idx: number) => { new Image().src = images[idx].fullUrl }
    preload((index + 1) % images.length)
    preload((index - 1 + images.length) % images.length)
  }, [index, images])

  const onTouchStart = useCallback(
    (e: React.TouchEvent) => { touchStartX.current = e.touches[0].clientX },
    [],
  )
  const onTouchEnd = useCallback(
    (e: React.TouchEvent) => {
      if (touchStartX.current === null) return
      const diff = touchStartX.current - e.changedTouches[0].clientX
      if (Math.abs(diff) > 50) diff > 0 ? next() : prev()
      touchStartX.current = null
    },
    [next, prev],
  )

  const current = images[index]
  // Hitung dimensi placeholder agar tidak ada layout shift saat thumbnail muncul
  const aspectRatio = current.width / current.height

  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/95"
      onClick={onClose}
      onTouchStart={onTouchStart}
      onTouchEnd={onTouchEnd}>
      <button
        className="absolute top-4 right-4 z-20 text-3xl text-white/70 hover:text-white"
        onClick={onClose}>
        ✕
      </button>

      {images.length > 1 && (
        <>
          <button
            className="absolute left-[2px] md:left-[calc(50%-430px)] top-1/2 z-30 -translate-y-1/2 rounded-full bg-blue-500/90 px-4 py-2 text-4xl font-bold text-white shadow-2xl hover:scale-110 hover:bg-blue-400 transition-all duration-200 hidden md:block"
            onClick={e => { e.stopPropagation(); prev() }}>
            ‹
          </button>
          <button
            className="absolute right-[2px] md:right-[calc(50%-430px)] top-1/2 z-30 -translate-y-1/2 rounded-full bg-blue-500/90 px-4 py-2 text-4xl font-bold text-white shadow-2xl hover:scale-110 hover:bg-blue-400 transition-all duration-200 hidden md:block"
            onClick={e => { e.stopPropagation(); next() }}>
            ›
          </button>
        </>
      )}

      {/*
        Progressive lightbox dengan dimensi eksplisit:
        - Container pakai aspect-ratio supaya tidak ada layout shift
        - Thumbnail (sudah di-cache dari grid) muncul instan sebagai blur placeholder
        - Full-res load di belakangnya; begitu selesai, thumbnail disembunyikan
      */}
      <div
        className="relative"
        style={{
          maxHeight: '90vh',
          maxWidth: '90vw',
          aspectRatio: String(aspectRatio),
          width: aspectRatio >= 1 ? '90vw' : `calc(90vh * ${aspectRatio})`,
        }}
        onClick={e => e.stopPropagation()}>
        <img
          key={`thumb-${index}`}
          src={current.url}
          alt={`Photo ${index + 1}`}
          loading="eager"
          decoding="async"
          className="absolute inset-0 h-full w-full object-contain blur-sm"
        />
        <img
          key={`full-${index}`}
          src={current.fullUrl}
          alt={`Photo ${index + 1}`}
          loading="eager"
          decoding="async"
          // @ts-expect-error fetchpriority valid HTML
          fetchpriority="high"
          className="absolute inset-0 h-full w-full object-contain"
          onLoad={e => {
            const thumb = e.currentTarget.previousElementSibling as HTMLElement | null
            if (thumb) thumb.style.display = 'none'
          }}
        />
      </div>
    </div>
  )
}

const MasonryGrid = memo(function MasonryGrid({
  images,
  title,
  onPhotoClick,
  eager = false,
  priority = false,
}: {
  images: ImageItem[]
  title: string
  onPhotoClick: (index: number) => void
  eager?: boolean
  priority?: boolean
}) {
  if (images.length === 0) {
    return (
      <div className="flex h-48 w-full items-center justify-center bg-gray-100">
        <span className="font-mono text-xs text-gray-400">no photo</span>
      </div>
    )
  }

  if (images.length === 1) {
    const {url, width, height} = images[0]
    return (
      <div
        className="relative w-full overflow-hidden bg-black"
        style={{paddingTop: `${(height / width) * 100}%`}}
        onClick={() => onPhotoClick(0)}>
        <ProgressiveImage
          src={url}
          alt={title}
          eager={eager}
          priority={priority}
          className="absolute inset-0 h-full w-full object-contain bg-black"
        />
      </div>
    )
  }

  if (images.length === 2) {
    return (
      <div className="grid grid-cols-2 gap-px overflow-hidden bg-gray-200">
        {images.map((img, i) => (
          <div
            key={i}
            className="relative h-48 overflow-hidden"
            onClick={() => onPhotoClick(i)}>
            <ProgressiveImage
              src={img.url}
              alt={`${title} ${i + 1}`}
              eager={eager}
              priority={priority && i === 0}
              className="absolute inset-0 h-full w-full object-cover"
            />
          </div>
        ))}
      </div>
    )
  }

  const isOdd = images.length % 2 !== 0
  const gridImages = isOdd ? images.slice(0, -1) : images
  const lastImage = isOdd ? images[images.length - 1] : null

  const col1: any[] = []
  const col2: any[] = []
  gridImages.forEach((img, i) => {
    ;(i % 2 === 0 ? col1 : col2).push({...img, origIndex: i})
  })

  return (
    <div className="flex flex-col gap-px overflow-hidden bg-gray-200">
      <div className="grid grid-cols-2 gap-px">
        {[col1, col2].map((col, colIdx) => (
          <div key={colIdx} className="flex flex-col gap-px">
            {col.map((img: any, i: number) => (
              <div
                key={i}
                className="relative h-48 overflow-hidden"
                onClick={() => onPhotoClick(img.origIndex)}>
                <ProgressiveImage
                  src={img.url}
                  alt={`${title} ${img.origIndex + 1}`}
                  eager={eager}
                  priority={priority && colIdx === 0 && i === 0}
                  className="absolute inset-0 h-full w-full object-cover"
                />
              </div>
            ))}
          </div>
        ))}
      </div>

      {lastImage && (
        <div
          className="relative h-48 overflow-hidden"
          onClick={() => onPhotoClick(images.length - 1)}>
          <ProgressiveImage
            src={lastImage.url}
            alt={`${title} ${images.length}`}
            eager={eager}
            className="absolute inset-0 h-full w-full object-cover"
          />
        </div>
      )}
    </div>
  )
})

export default function Gallery() {
  const {galleries, did} = useLoaderData<typeof loader>()

  const [visibleCount, setVisibleCount] = useState(INITIAL_GALLERY_LIMIT)
  const [lightbox, setLightbox] = useState<{
    images: ImageItem[]
    index: number
  } | null>(null)

  const visibleGalleries = galleries.slice(0, visibleCount)
  const loadMoreRef = useRef<HTMLDivElement | null>(null)

  useEffect(() => {
    if (!loadMoreRef.current) return
    const observer = new IntersectionObserver(
      entries => {
        if (entries[0].isIntersecting) {
          setVisibleCount(prev => Math.min(prev + 6, galleries.length))
        }
      },
      {rootMargin: '800px'},
    )
    observer.observe(loadMoreRef.current)
    return () => observer.disconnect()
  }, [galleries.length])

  return (
    <article className="container mx-auto px-6 pb-24 pt-12 md:pt-20">
      {lightbox && (
        <Lightbox
          images={lightbox.images}
          initialIndex={lightbox.index}
          onClose={() => setLightbox(null)}
        />
      )}

      <header className="mb-12 flex max-w-prose flex-col gap-5 md:mb-16">
        <h1 className="font-display text-4xl leading-[1.02] text-950 md:text-6xl">
          Gallery
        </h1>
        <p className="font-sans text-lg text-500">
          Sometimes you see me. Mostly, you see what I see
        </p>
      </header>

      {visibleGalleries.length === 0 ? (
        <div className="max-w-prose">
          <p className="font-sans text-lg text-500">Belum ada gallery.</p>
        </div>
      ) : (
        <>
          <div className="columns-1 gap-4 space-y-4 md:columns-2 lg:columns-3">
            {visibleGalleries.map((gallery: any, index: number) => {
              const value = gallery.value
              const rkey = gallery.uri.split('/').at(-1)

              return (
                <div
                  key={gallery.uri}
                  className="group block break-inside-avoid overflow-hidden rounded-md border border-100 bg-50 transition-colors hover:border-300">
                  <MasonryGrid
                    images={gallery.images}
                    title={value.title ?? 'Gallery'}
                    eager={index < 6}
                    priority={index === 0}
                    onPhotoClick={photoIndex =>
                      setLightbox({images: gallery.images, index: photoIndex})
                    }
                  />

                  <a
                    href={`https://grain.social/profile/${did}/gallery/${rkey}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="block p-5">
                    <div className="flex flex-col gap-2">
                      <h2 className="font-display text-xl text-950 transition-colors group-hover:text-600">
                        {value.title ?? 'Untitled'}
                      </h2>

                      {value.address ? (
                        <p className="font-sans text-sm text-500">
                          {value.address.locality}
                          {value.address.region
                            ? ', ' + value.address.region
                            : ''}
                        </p>
                      ) : null}

                      <div className="mt-auto flex items-center justify-between pt-2">
                        <p className="font-mono text-xs text-300">
                          {new Date(value.createdAt).toLocaleDateString(
                            'id-ID',
                            {year: 'numeric', month: 'long', day: 'numeric'},
                          )}
                        </p>
                        <p className="font-mono text-xs text-300">
                          {gallery.images.length} foto
                        </p>
                      </div>
                    </div>
                  </a>
                </div>
              )
            })}
          </div>

          <div
            ref={loadMoreRef}
            className="flex h-24 items-center justify-center">
            {visibleCount < galleries.length && (
              <div className="h-8 w-8 animate-spin rounded-full border-2 border-gray-300 border-t-transparent" />
            )}
          </div>
        </>
      )}
    </article>
  )
}
