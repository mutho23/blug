...
// Ganti nilai ini sesuai selera. Semakin kecil angka = semakin cepat load,
// tapi kualitas gambar semakin turun.
const THUMB_WIDTH = 600   // px — lebar thumbnail di grid
const THUMB_QUALITY = 65  // 1–100 — kualitas thumbnail di grid
const FULL_WIDTH = 1800   // px — lebar gambar di lightbox
const FULL_QUALITY = 85   // 1–100 — kualitas gambar di lightbox
...
function toWsrv(
  ...
  const params = new URLSearchParams({
    ...
    w: String(opts.w),
    q: String(opts.q),
    output: 'webp', // konversi ke WebP otomatis
    we: '1',        // strip metadata EXIF
  })
...
type ImageItem = {
  url: string     // thumbnail (wsrv.nl, kecil) — dipakai di grid
  fullUrl: string // full-res (wsrv.nl, besar) — dipakai di lightbox
  width: number
  height: number
}
...
export const loader = async () => {
  ...
  return json(
    ...
    {
      headers: {
        // Buka koneksi TCP/TLS ke kedua domain sedini mungkin
        Link: [
...
function ProgressiveImage({
  ...
  // Gambar yang sudah di-cache browser tidak memicu onLoad,
  // sehingga tanpa ini gambar akan stuck di opacity-0 selamanya.
  useEffect(() => {
...
function Lightbox({
  ...
  // Preload prev & next — bukan hanya next — supaya navigasi terasa instan
  useEffect(() => {
  ...
  return (
    <div
      ...
      {/*
        ...
        - Thumbnail (url) sudah ada di browser cache dari grid → muncul instan sebagai blur placeholder
        - Full-res (fullUrl) diload di balik thumbnail → begitu selesai, thumbnail disembunyikan
        Hasilnya: tidak ada layar hitam kosong saat menunggu gambar besar selesai diunduh.
      */}
      <div
...
export default function Gallery() {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      {
        rootMargin: '800px', // trigger lebih awal supaya tidak ada jeda saat scroll
      },
    )
